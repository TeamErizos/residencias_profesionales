...
<body>
    ...
    <div class="btn">
        <?php
        try {
            // Llamar a la función obtenerPromedioCalificaciones
            $resultados = $calificaciones->obtenerPromedioCalificaciones($p_x_a_id);

            // Comprobar si ambas calificaciones existen
            if (isset($resultados['calif_reporte_asesor_interno']) && isset($resultados['calif_reporte_asesor_externo'])) {
                // Calificaciones existen, proceder a la generación de PDF y otras operaciones
                implode(", ", $resultados['calif_reporte_asesor_interno']);
                implode(", ", $resultados['calif_reporte_asesor_externo']);

                // Mostrar promedio 
                $resultados['promedio'];

                // Generar formulario para cerrar y proceder al siguiente archivo
                echo "<form action=\"create_P_X_A_file.php\" method=\"post\">";
