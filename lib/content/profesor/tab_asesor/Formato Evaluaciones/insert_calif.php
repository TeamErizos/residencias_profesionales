...
class Calificaciones
{
    ...
    public function obtenerPromedioCalificaciones($p_x_a_id)
    {
        ...
        if (isset($registro['calif_parcial1_asesor_interno'], $registro['calif_parcial1_asesor_externo'], $registro['calif_parcial2_asesor_interno'], $registro['calif_parcial2_asesor_externo'])) {
            ...
            $promedio_p1 = ($calif_inter_p1 + $calif_exter_p1) / 2;

            $promedio_p2 = ($calif_inter_p2 + $calif_exter_p2) / 2;
            ...
            return array(
                ...
                'sumaInterno_p1' => $calif_inter_p1,
                'sumaInterno_p2' => $calif_inter_p2,
                'sumaExterno_p1' => $calif_exter_p1,
                'sumaExterno_p2' => $calif_exter_p2
            );
        } else {
    ...
    function insertarEvaRepoFinal($id_p_x_a, $ReporteFinal_file)
    {
        ...
        try {
            ...
            $sql = "SELECT insertar_evaluacion_y_seguimiento(:id_p_x_a, :ReporteFinal_file)";
    ...
    function descargarReporteFinal($id_p_x_a)
    {
        ...
        $query = "SELECT evaluacion_y_seguimiento FROM documentos WHERE id_proyecto_activo = :id_p_x_a";
        $stmt = $this->conn->prepare($query);
        ...
        if ($result) {
            // Extraer el reporte
            $ArchivoReporte = $result['evaluacion_y_seguimiento'];

            // Verificar si el reporte se extrajo correctamente
            if ($ArchivoReporte) {

                // Ruta del pdf
                $file = "Docs/" . $id_p_x_a . "_Evaluación_Seguimiento.pdf";

                // Guardar el reporte en un archivo
                file_put_contents($file, $ArchivoReporte);

                // Verificar si el archivo se guardó correctamente
                if (file_exists($id_p_x_a . "_Evaluación_Seguimiento.pdf")) {
    ...
    {
        try {
            $query = "SELECT calif_parcial1_asesor_externo, calif_parcial1_asesor_interno, calif_parcial2_asesor_interno, calif_parcial2_asesor_externo
            ...
            if ($resultados) {
                $calificaciones_externo_p1 = str_replace(array('{', '}'), '', $resultados['calif_parcial1_asesor_externo']);
                $calificaciones_externo_p1_array = explode(',', $calificaciones_externo_p1);

                $calificaciones_interno_p1 = str_replace(array('{', '}'), '', $resultados['calif_parcial1_asesor_interno']);
                $calificaciones_interno_p1_array = explode(',', $calificaciones_interno_p1);

                $calificaciones_externo_p2 = str_replace(array('{', '}'), '', $resultados['calif_parcial2_asesor_externo']);
                $calificaciones_externo_p2_array = explode(',', $calificaciones_externo_p2);

                $calificaciones_interno_p2 = str_replace(array('{', '}'), '', $resultados['calif_parcial2_asesor_interno']);
                $calificaciones_interno_p2_array = explode(',', $calificaciones_interno_p2);

                return array(
                    'calificaciones_externo_p1' => $calificaciones_externo_p1_array,
                    'calificaciones_interno_p1' => $calificaciones_interno_p1_array,
                    'calificaciones_externo_p2' => $calificaciones_externo_p2_array,
                    'calificaciones_interno_p2' => $calificaciones_interno_p2_array
                );
            } else {
